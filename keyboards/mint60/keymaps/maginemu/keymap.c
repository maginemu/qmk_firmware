...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [BASE] = LAYOUT(                                                         \
    ...
    KC_GRV,    KC_CAPS,    KC_LALT,    KC_LGUI,    KC_SPC,  TD(DANCE_EASY_NUMS), TD(DANCE_EASY_SYMBOLS),  KC_RGUI,KC_LEFT,KC_DOWN,KC_RGHT \
  ),
  [SHIFTED_BASE] = LAYOUT(                                                         \
  ...
  [EASY_NUMS] = LAYOUT( \
    ...
    _______,       XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,   _______, _______, KC_LBRC, KC_RBRC, _______, _______, XXXXXXX, _______, \
  ...
  [EASY_SYMBOLS] = LAYOUT( \
    ...
    _______,       XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,   KC_LPRN, KC_RPRN, S(KC_LBRC), S(KC_RBRC), _______, _______, XXXXXXX, _______, \
    XXXXXXX,   _______,    _______,    _______,    KC_LSFT,       XXXXXXX, KC_RSFT, _______,                   XXXXXXX, XXXXXXX, XXXXXXX \
  ),
  [MOVE] = LAYOUT( \
    XXXXXXX,  XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,   XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,  XXXXXXX,  XXXXXXX, \
...
void dance_nums_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (dance_nums_state.state) {
    ...
    case DOUBLE_TAP: register_code(KC_GRV); unregister_code(KC_GRV);register_code(KC_GRV); break;
    case DOUBLE_HOLD: layer_on(MOVE); break;
    case DOUBLE_SINGLE_TAP: register_code(KC_GRV); unregister_code(KC_GRV); register_code(KC_GRV); break;
    case TRIPLE_TAP: register_code(KC_GRV); unregister_code(KC_GRV); register_code(KC_GRV);unregister_code(KC_GRV); register_code(KC_GRV); break;
...
void dance_nums_reset (qk_tap_dance_state_t *state, void *user_data) {
  switch (dance_nums_state.state) {
    ...
    case DOUBLE_TAP: unregister_code(KC_GRV); break;
    case DOUBLE_HOLD: layer_off(MOVE); break;
    case DOUBLE_SINGLE_TAP: unregister_code(KC_GRV); break;
    case TRIPLE_TAP: unregister_code(KC_GRV); break;
  }
...
void dance_symbols_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (dance_symbols_state.state) {
    ...
    case DOUBLE_TAP: register_code(KC_BSLS); unregister_code(KC_BSLS); register_code(KC_BSLS); break;
    ...
    case TRIPLE_TAP: register_code(KC_BSLS); unregister_code(KC_BSLS); register_code(KC_BSLS);unregister_code(KC_BSLS); register_code(KC_BSLS); break;
    //Last case is for fast typing. Assuming your key is `f`:
    //For example, when typing the word `buffer`, and you want to make sure that you send `ff` and not `Esc`.
    //In order to type `ff` when typing fast, the next character will have to be hit within the `TAPPING_TERM`, which by default is 200ms.
  }
